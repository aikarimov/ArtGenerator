...
namespace ArtModel.Tracing
{
    ...
    public class Tracer : IEnumerable<(ArtBitmap, ArtBitmap)>
    {
                ...
                void MakeStroke()
                {
                    ...
                    void CreateTask(int index, int direction)
                    {
                        ...
                        tasks[i] = Task.Run(() =>
                        {
                            ...
                            results[i].dispersion = CalculateStrokeCanvasError(stroke, tracingResult, coordinates);
                        });
                    }
                    ...
                    if (finalResult.dispersion > localData.DispersionRejectBound)
                        return;
        ...
        private TracingResult GetSegmentedTracePath(ArtGeneration genData, ArtBitmap bitmap, (int x, int y) startingPoint, double[,] brightnessMap, int direction)
        {
            ...
            for (int segmentPoint = 2; segmentPoint <= _points; segmentPoint++)
            {
                ...
                for (int i = tracingPaths.Length - 1; i >= 0; i--)
                {
                    ...
                    if (tpath.SquaredError <= tolerance)
                    {
                        ...
                        else
                        {
                            ...
                            tracingResult.Dispersion = tpath.SquaredError;
                            ...
                            double fraction = (segmentedLength - tpath.Length) * 100 / segmentedLength;
                            if ((fraction < 10 || fraction > 90) && segmentPoint > 2)
                                break;

                            tracingResult.SP.SetP(StrokeProperty.Fraction, fraction);
        ...
        private double CalculateStrokeCanvasError(Stroke stroke, TracingResult tracingResult, (int x, int y) globalPoint)
        {
        ...
        private void WritePixelsRender(Stroke stroke, TracingResult tracingResult, (int x, int y) globalPoint, IPointDecider decider = null)
        {
                ...
                for (int y = 0; y < stroke.Height; y++)
                {
                    ...
                    if (_artificialRender.IsInside(globalX, globalY))
                    {
                        ...
                        if (strokeAlpha < Stroke.BLACK_BORDER_MEDIUM)
                        {
                            ...
                            {
                                var colorToDraw = GraphicsMath.CalculateAlpha(_artificialRender[globalX, globalY], tracingResult.MeanColor, (255.0 - strokeAlpha) / 255.0);
                                _artificialRender[globalX, globalY] = colorToDraw;
                            }
            ...
            //_artificialModel.Save("C:\\Users\\skura\\source\\repos\\ArtGenerator\\Output\\Images", $"Render_{drawDisp}");
            //_artificialRender.Save("C:\\Users\\skura\\source\\repos\\ArtGenerator\\Output\\Images", $"Model_{drawDisp}");
